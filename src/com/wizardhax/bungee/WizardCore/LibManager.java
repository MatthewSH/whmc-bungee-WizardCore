...
public class LibManager {
	...
	public LibManager(Plugin plugin, String[] libs) {
		for (String lib : libs) {
			try {

				final File[] a = new File[] { new File(plugin.getDataFolder(), lib) };

				for (File f : a) {
					if (!f.exists()) {
						extractFromJar(f.getName(), f.getAbsolutePath());
					} else {
				...
				for (final File f : a) {
					if (!f.exists()) {
						...
						System.out.printf("Could not find library: " + f.getName());
						return;
					}
					addClassPath(getJarUrl(f));
				}
	...
	public static JarFile getRunningJar() throws IOException {
		...
		String path = new File(LibManager.class.getProtectionDomain().getCodeSource().getLocation().getPath())
				.getAbsolutePath();
	...
	static {
		final URL resource = LibManager.class.getClassLoader().getResource("plugin.yml");
		if (resource != null) {
